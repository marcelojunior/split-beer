  ...
  <div>
    <LogoHeader>
      <template v-slot:actions>
        <v-btn variant="text" color="orange darken-1" to="/">{{ $t('createList') }}</v-btn>
      ...
      <v-card-text>
        ...
        <div v-else class="text-center">
          ...
          <p class="text-h4 font-weight-bold">
            ...
            <v-btn size="x-small" variant="outlined" icon="fa fa-pencil" @click="editingValue = true"></v-btn>
          </p>
        ...
        <!-- NOVO NOME -->
        <div>
          <h5 class="text-h5 font-weight-light text-center mb-3">{{ $t('addName') }}</h5>
          <v-text-field v-model="newName.name" :label="$t('yourName')" :rules="requiredRule"
            autocomplete="do-not-autofill" variant="solo"></v-text-field>
          <DecimalField v-model="newName.value" :label="$t('sponsored')" :hint="$t('sponsoredTip')" persistent-hint
            variant="solo"></DecimalField>
          <v-btn color="success" :disabled="!newNameValid" class="mt-4" prepend-icon="fa fa-plus" @click="addName">
            {{
                $t('save')
            }}
          </v-btn>
        ...
        <!-- LISTA DE NOMES -->
        <v-expansion-panels>
          <v-expansion-panel v-for="(item, index) in list.items" :key="item.id" :selected="updateValueToPay(item)">
                ...
                <v-col>
                  <div class="text-right mr-2">
                    ...
                    <div v-else-if="list.value">
                      <span class="text-disabled">{{ $n(eachValue, 'currency') }}</span>
                    </div>
                  </div>
                </v-col>
              </v-row>
            ...
            <v-expansion-panel-text>
              <div v-if="!item.value && list.value" class="mb-3">
                <v-btn color="success" class="mt-2" prepend-icon="fa fa-check" :disabled="!item.valueToPay" size="large"
                  :loading="paying" @click="pay(item)" block>
                  {{ $t('paidN', { n: item.valueToPay }) }}
                </v-btn>
              </div>
              <v-btn prepend-icon="fa fa-trash" color="error" size="small" @click="deleteName(item)" variant="outlined"
                block>{{ $t('delete') }}</v-btn>
            </v-expansion-panel-text>
          </v-expansion-panel>
        </v-expansion-panels>

        <div v-if="list.items && list.items.length > 0">
          <v-btn variant="outlined" class="mt-3" prepend-icon="fa fa-whatsapp" @click="shareList()" block>
    ...
    <v-snackbar v-model="snackbar" :timeout="3000">
      ...
      <template v-slot:actions>
        <v-btn icon="fa fa-close" size="small" @click="snackbar = false"></v-btn>
      </template>
    </v-snackbar>
  </div>
</template>
...
export default defineComponent({
  name: 'ListView',
  components: { BasicInputs, DecimalField, MyLastList, SelectLocale, LogoHeader },
  data() {
    return {
      editingValue: false,
      newNameValid: false,
  ...
  methods: {
    ...
    updateValueToPay(item: ListItem) {
      item.valueToPay = this.$n((item.value ?? this.eachValue) as number, 'currency');
    },
    pay(item: ListItem) {
